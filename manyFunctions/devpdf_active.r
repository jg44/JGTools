.devpdf <- function(file,
                    ...
                    pathtoMDeditor = NULL,
                    currdir = getwd()) {
  suppressWarnings(largestPdf <- as.numeric(gsub("\\D", "", substring(
    list.files(pdfPath), 1, 2
  ))))
  mxCur <- ifelse(((all(
    is.na(largestPdf)
  )) || length(largestPdf) == 0), 0, max(largestPdf, na.rm = TRUE))
  ...
  projNamePath <- paste0("./", substring(
    tf,
    nchar(rstudioapi::getActiveProject()) - nchar(projName),
    nchar(tf)
  ))
  ...
  #     ff <- paste0(nextNum, "_", ff)
  if (!any(list.dirs() == "./notesGraphAnnot"))
    dir.create("./notesGraphAnnot") #else print("")
  #overwrite <- FALSE
  ...
  if (overwrite || (!fileExists)) {
    ...
    if (png) {
      # Read the ImageMagick executable path from file
      if (file.exists("~/AppData/Roaming/magickPath.txt")) magick_path <-
          trimws(readLines("~/AppData/Roaming/magickPath.txt", warn = FALSE)[1]) else print("Set the path to ImageMagick using .set_magick_path()")

      if (!file.exists(magick_path)) {
        stop("ImageMagick path is missing or incorrect. Set with .set_magick_path()")
      }
      pngfile <- .pdf2png(paste0(pdfPath, "/", ff), magick_path = magick_path)
    } else
      if (!overwrite & fileExists) {
    ...
    if ((meta) && (tf != "n")) {
      tmpMaxannotNum <- as.numeric(gsub("\\D", "", substring(
        list.files(annotationsPath, pattern = "*.annot.md"), 1, 2
      )))
      if (all(is.na(tmpMaxannotNum)))
        mxCur <- 0
      else
        mxCur <- max(tmpMaxannotNum, na.rm = TRUE)

      nextNum <- formatC(mxCur,
                         width = 2,
                         format = "d",
                         flag = "0")
      annotFilenameCheck <- paste0(nextNum, "_", ff, ".annot.md")
      ...
      if (file.exists(paste0(annotationsPath, annotFilenameCheck)) &
          (overwrite))
        annotFilename <- annotFilenameCheck
      else
        annotFilename <-
        paste0(ff, ".annot.md")
    ...
    if (history) {
      sink(paste0(annotationsPath, "ProjectHistoryFile.md"),
           append = TRUE)
      cat(paste0("\n### Script file: ", tf))
      cat("\n")
      cat("\nTimestamp: ", date(), "\n")
