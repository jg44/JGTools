...
fig_save <- function(
  ...
  if (is.null(plot_obj))
    stop("No plot supplied and no last_plot() available.")
  ...
  # ---- simple numbering ----
  existing <- list.files(file.path(root, pdf_dir), pattern = "^[0-9]+")
  next_num <- if (length(existing) == 0) 1 else {
    nums <- as.numeric(sub("^([0-9]+).*", "\\1", existing))
    max(nums, na.rm = TRUE) + 1
  }

  base_name <- sprintf("%02d_%s", next_num, slug)

  pdf_file  <- file.path(root, pdf_dir, paste0(base_name, ".pdf"))
  png_file  <- file.path(root, img_dir, paste0(base_name, ".png"))
  annot_file <- file.path(root, paste0(base_name, ".md"))

  # ---- capture current device size ----
